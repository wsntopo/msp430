 ...
 *   Target : Texas Instruments Launchpad
 *            "MSP430G2553"
 *   Radios : CC2500
 * ~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=
 */
...
#define __mrfi_GDO2_SEL__						P1SEL
#define __mrfi_GDO2_IN__						P1IN
#define __mrfi_GDO2_IE__						P1IE
#define __mrfi_GDO2_IFG__						P1IFG
#define __mrfi_GDO2_IES__						P1IES
#define MRFI_GDO2_INT_VECTOR                  PORT1_VECTOR

#define MRFI_CONFIG_GDO2_PIN_AS_INPUT()       st( __mrfi_GDO2_SEL__ &= ~BV(__mrfi_GDO2_BIT__); ) /* clear pin special function default */
#define MRFI_GDO2_PIN_IS_HIGH()               (__mrfi_GDO2_IN__ & BV(__mrfi_GDO2_BIT__))

#define MRFI_ENABLE_GDO2_INT()                st( __mrfi_GDO2_IE__  |=  BV(__mrfi_GDO2_BIT__); ) /* atomic operation */
#define MRFI_DISABLE_GDO2_INT()               st( __mrfi_GDO2_IE__  &= ~BV(__mrfi_GDO2_BIT__); ) /* atomic operation */
#define MRFI_GDO2_INT_IS_ENABLED()             (  __mrfi_GDO2_IE__  &   BV(__mrfi_GDO2_BIT__) )
#define MRFI_CLEAR_GDO2_INT_FLAG()            st( __mrfi_GDO2_IFG__ &= ~BV(__mrfi_GDO2_BIT__); ) /* atomic operation */
#define MRFI_GDO2_INT_FLAG_IS_SET()            (  __mrfi_GDO2_IFG__ &   BV(__mrfi_GDO2_BIT__) )
#define MRFI_CONFIG_GDO2_RISING_EDGE_INT()    st( __mrfi_GDO2_IES__ &= ~BV(__mrfi_GDO2_BIT__); ) /* atomic operation */
#define MRFI_CONFIG_GDO2_FALLING_EDGE_INT()   st( __mrfi_GDO2_IES__ |=  BV(__mrfi_GDO2_BIT__); ) /* atomic operation */
...
#define __mrfi_SPI_CSN_DIR__				P2DIR
#define __mrfi_SPI_CSN_SEL__				P2SEL
#define __mrfi_SPI_CSN_OUT__				P2OUT
#define MRFI_SPI_CONFIG_CSN_PIN_AS_OUTPUT()   st( __mrfi_SPI_CSN_DIR__ |=  BV(__mrfi_SPI_CSN_GPIO_BIT__); \
						__mrfi_SPI_CSN_SEL__ &= ~BV(__mrfi_SPI_CSN_GPIO_BIT__); ) 
#define MRFI_SPI_DRIVE_CSN_HIGH()             st( __mrfi_SPI_CSN_OUT__ |=  BV(__mrfi_SPI_CSN_GPIO_BIT__); ) /* atomic operation */
#define MRFI_SPI_DRIVE_CSN_LOW()              st( __mrfi_SPI_CSN_OUT__ &= ~BV(__mrfi_SPI_CSN_GPIO_BIT__); ) /* atomic operation */
#define MRFI_SPI_CSN_IS_HIGH()                 (  __mrfi_SPI_CSN_OUT__ &   BV(__mrfi_SPI_CSN_GPIO_BIT__) )
